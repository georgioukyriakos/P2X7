# Molecular dynamics snapshots of P2X7R–antagonist complexes
...
```
structures/
├── hP2X7_altp1_initial_frame.pdb      hP2X7_altp2_initial_frame.pdb
├── hP2X7_altp1_last_frame_rep_1.pdb   hP2X7_altp2_last_frame_rep_1.pdb
├── hP2X7_altp1_last_frame_rep_2.pdb   hP2X7_altp2_last_frame_rep_2.pdb
├── hP2X7_mbx47_initial_frame.pdb      mP2X7_altp2_initial_frame.pdb
├── hP2X7_mbx47_last_frame_rep_1.pdb   mP2X7_altp2_last_frame_rep_1.pdb
├── hP2X7_mbx47_last_frame_rep_2.pdb   mP2X7_altp2_last_frame_rep_2.pdb
├── rP2X7_altp2_initial_frame.pdb
├── rP2X7_altp2_last_frame_rep_1.pdb
└── rP2X7_altp2_last_frame_rep_2.pdb
```

15 files — 5 complexes × (1 initial frame + the last frame of each of the 2 replicates).

### File-naming convention

```
<species>P2X7_<ligand>_<frame>[_rep_<n>].pdb
    │        │        │         │
    │        │        │         └── independent replicate (random initial velocities)
    │        │        └──────────── initial_frame = 0 ns · last_frame = 500 ns
...
<details open>
<summary><b>System preparation</b></summary>

<br>

- Cryo-EM structures prepared with the **Protein Preparation Wizard** (Schrödinger Suite): bond orders and disulfides assigned, missing hydrogens and loops added with **Prime**, N-/C-termini capped with acetyl and *N*-methylamino groups.
- Ligand ionisation states at pH 7.5 assigned with **Epik**.
- All-atom minimisation with **OPLS2005**, heavy-atom RMSD restrained to 0.30 Å.
- Receptor inserted into a pre-equilibrated hydrated **POPC** bilayer, oriented per the **OPM** database, with a 30 Å buffer to the orthorhombic box edge along all axes.
- **TIP3P** water; Na⁺/Cl⁻ added to neutralise and reach 0.150 M NaCl.

| Parameter           | Value              |
|---------------------|--------------------|
| Box dimensions      | 162 × 162 × 217 Å³ |
| Total atoms         | ~542,000           |
| POPC lipids         | 711                |
| Water molecules     | ~139,300           |
| Boundary conditions | periodic           |

</details>

<details open>
<summary><b>Force fields</b></summary>
...
| Protein                     | amber99sb                                        |
| Lipids                      | CHARMM36                                         |
| Ligands                     | GAFF (parameterised with `antechamber`, AMBER18) |
| Water                       | TIP3P                                            |
| Intermolecular interactions | amber99sb                                        |

Parameters were assigned with the `viparr` tool in **Desmond v5.3**.

</details>

<details open>
<summary><b>Equilibration and production</b></summary>

<br>

Six-step Desmond equilibration protocol:

| Step | Ensemble                              | T (K) | Length | Restraints on non-H solute atoms |
|------|---------------------------------------|-------|--------|----------------------------------|
| 1    | NVT (Brownian)                        | 10    | 100 ps | 50 kcal mol⁻¹ Å⁻²                |
| 2    | NPT (Berendsen thermostat)            | 10    | 12 ps  | 50 kcal mol⁻¹ Å⁻²                |
| 3    | NPT (Berendsen thermostat + barostat) | 10    | 12 ps  | 50 kcal mol⁻¹ Å⁻²                |
| 4    | NPT                                   | 300   | 24 ps  | 50 kcal mol⁻¹ Å⁻²                |
| 5    | NPT                                   | 300   | 24 ps  | released                         |

**Production:** 500 ns per replicate in the NPT ensemble at **310 K / 1 atm**, Nosé–Hoover chain thermostat and Martyna–Tobias–Klein barostat. Long-range electrostatics by **PME** (0.8 Å grid); van der Waals and short-range electrostatics smoothly truncated at 9.0 Å. **r-RESPA** integrator with a 2 fs inner time step (bonded and non-bonded within 12 Å) and a 6 fs outer time step; **SHAKE** constraints on heavy-atom–hydrogen bonds. Frames saved every 10 ps. **Two replicates** per system, each seeded with random velocities; the last frame of each replicate is provided here.

Simulations were run with the GPU implementation of Desmond.
...
**Molecular dynamics simulations** — Kyriakos Georgiou, Efpraxia Tzortzini and Antonios Kolocouris, Laboratory of Medicinal Chemistry, Department of Pharmacy, National and Kapodistrian University of Athens

**Corresponding authors of the study** — Steven E. Mansoor (mansoors@ohsu.edu) and Santiago Vázquez (svazquez@ub.edu)

---

## License

Released under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) — reuse and adaptation are permitted with attribution.
